...
static void SetupCliArgs(ArgsManager &argsman) {
    ...
    argsman.AddArg(
        ...
        "corresponding -wallet option passed to ausd). This changes the "
...
static int AppInitRPC(int argc, char *argv[]) {
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (gArgs.IsArgSet("-version")) {
            ...
            strUsage += "\n"
                        "Usage:  aus-cli [options] <command> [params]  "
                        "Send command to " PACKAGE_NAME "\n"
                        "or:     aus-cli [options] -named <command> "
                        "[name=value]...  Send command to " PACKAGE_NAME
                        " (with named arguments)\n"
                        "or:     aus-cli [options] help                "
                        "List commands\n"
                        "or:     aus-cli [options] help <command>      Get "
                        "help for a command\n";
...
public:
    ...
    UniValue ProcessReply(const UniValue &batch_in) override {
        ...
        if (networkinfo["version"].getInt<int>() < 230000) {
            throw std::runtime_error("-netinfo requires ausd server to be "
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(
            strprintf("Could not connect to the server %s:%d%s\n\nMake sure "
                      "the ausd server is running and that you are "

...
The official ausd guide to backporting code from Bitcoin Core. When searching
for items to backport, especially be on the lookout for bug fixes, code cleanup, and
important refactors, as these help improve ausd despite consensus-level differences
...
3. Tag the fork commit as `fork-commit`. ausd was forked from Bitcoin Core
...
Before you begin backporting commits, you will need to add an additional remote to your ausd repo.
...
The ausd monorepo includes subprojects that are stored in subdirectories of the main repository.

...
shutdown_bitcoind() {
  ...
  do
    ...
    then
      echo "Timed out waiting for ausd to stop"
      exit 3
    fi
    sleep 0.5
  done
...
do
  ...
  then
    echo "Timed out waiting for ausd to start"
    exit 2
  fi
  sleep 0.5
done

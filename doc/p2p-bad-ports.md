When ausd automatically opens outgoing P2P connections it chooses
...
A malicious actor may gossip an address:port where no ausd node is
listening, or one where a service is listening that is not related to the ausCash
...
and usually require authentication. A connection attempt (by ausd, trying
...
Below is a list of "bad" ports which ausd avoids when choosing a peer to

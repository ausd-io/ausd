...
Below are some notes on how to build ausd for Windows.

The options known to work for building ausd on Windows are:

* On Linux, using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Debian Bullseye is recommended
and is the platform used to build the ausd Windows release binaries.
...
The cmake version packaged with Ubuntu Bionic is too old for building Building ausd.
...
Note that for WSL the ausd source path MUST be somewhere in the default mount file system, for
...
It's not possible to build the ausd code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the ausd source code).

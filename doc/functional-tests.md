...
The [/test/](/test/) directory contains integration tests that test ausd
...
ausd and aus-qt by interacting with them through the RPC and P2P
interfaces.
...
The P2P and RPC ports used by the ausd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another ausd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its ausd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other ausd processes are running.

On linux, the test framework will warn if there is another
ausd process running when the tests are started.

If there are zombie ausd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
ausd processes running on the system, so should not be used if any non-test
ausd processes are being run.**
...
sure ausd processes are stopped as above):
...
- When run by our CI, no logs are output to the console. However, if a test
  fails, the `test_framework.log` and ausd `debug.log`s will all be dumped
...
`test_framework.log` and ausd `debug.log`s can be combined into a single
...
call methods that interact with the ausd nodes-under-test.

If further introspection of the ausd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/ausd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/ausd <pid>
```
...
with the ausd(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a ausd.  `P2PInterface`
...
Test objects for interacting with a ausd node over the p2p interface.

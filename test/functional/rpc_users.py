...
class HTTPBasicsTest(BitcoinTestFramework):
    ...
    def setup_chain(self):
        super().setup_chain()
        # Append rpcauth to auscash.conf before initialization
        ...
        with open(
            os.path.join(get_datadir_path(self.options.tmpdir, 0), "auscash.conf"),
            "a",
            encoding="utf8",
        ) as f:
            f.write(f"{rpcauth}\n")
            ...
            f.write(f"{rpcauth3}\n")
        with open(
            os.path.join(get_datadir_path(self.options.tmpdir, 1), "auscash.conf"),
            "a",
            encoding="utf8",
        ) as f:
